import { CANVAS_WIDTH, CANVAS_HEIGHT, GAME_STATES, COLORS } from './utils/constants.js';
...
export class Game {
  ...
  showMenu() {
    this.state = GAME_STATES.MENU;
    this.renderer.clear();
    this.renderer.drawText('DIG DUG', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 60, {
      size: 16,
      color: COLORS.TEXT_YELLOW,
      align: 'center'
    });
    this.renderer.drawText('PRESS SPACE', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 10, {
      size: 8,
      color: COLORS.TEXT_WHITE,
      align: 'center'
    });
    this.renderer.drawText('TO START', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 30, {
      size: 8,
      color: COLORS.TEXT_WHITE,
      align: 'center'
    });
    this.renderer.drawText('ARROWS MOVE', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 60, {
      size: 6,
      color: COLORS.TEXT_WHITE,
      align: 'center'
    });
    this.renderer.drawText('SPACE PUMP', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 75, {
      size: 6,
      color: COLORS.TEXT_WHITE,
      align: 'center'
    });
  ...
  checkCollisions() {
    ...
    this.enemies = this.enemies.filter(enemy => {
      if (this.collisionSystem.checkPlayerEnemyCollision(this.player, enemy)) {
        if (enemy.isInflating) {
          // Enemy defeated
          const points = this.scoreManager.addEnemyKill(enemy.type, enemy.distanceFromPlayer);
    ...
    this.rocks.forEach(rock => {
      if (rock.isFalling) {
        // Check rock-enemy
        this.enemies = this.enemies.filter(enemy => {
          if (this.collisionSystem.checkRockEntityCollision(rock, enemy)) {
        ...
        // Check rock-player
        if (this.collisionSystem.checkRockEntityCollision(rock, this.player)) {
          this.playerHit();
        }
      }
    });
    ...
    this.bonusItems = this.bonusItems.filter(item => {
      if (this.collisionSystem.checkPlayerBonusCollision(this.player, item)) {
  ...
  renderLevelComplete() {
    this.render();
    this.renderer.drawText('LEVEL', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 10, {
      size: 12,
      color: COLORS.TEXT_YELLOW,
      align: 'center'
    });
    this.renderer.drawText('COMPLETE', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 10, {
      size: 12,
      color: COLORS.TEXT_YELLOW,
      align: 'center'
    });
  ...
  renderGameOver() {
    this.renderer.clear();
    this.renderer.drawText('GAME OVER', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 40, {
      size: 12,
      color: COLORS.TEXT_YELLOW,
      align: 'center'
    });
    ...
    this.renderer.drawText(`${this.scoreManager.score}`, CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 20, {
      size: 10,
      color: COLORS.TEXT_YELLOW,
      align: 'center'
    });
    this.renderer.drawText('PRESS SPACE', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 50, {
      size: 6,
      color: COLORS.TEXT_WHITE,
      align: 'center'
    });
    this.renderer.drawText('TO RESTART', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 65, {
      size: 6,
      color: COLORS.TEXT_WHITE,
      align: 'center'
    });
