import { GRID_WIDTH, GRID_HEIGHT, TILE_SIZE, LEVEL, TILE_TYPES } from '../utils/constants.js';
...
export class LevelManager {
  ...
  placeRocksAfterEnemies(levelNumber, enemies) {
    const targetRocks = Math.min(LEVEL.ROCKS_PER_LEVEL + Math.floor(levelNumber / 3), 6);
    ...
    for (let i = 0; i < targetRocks; i++) {
      ...
      while (!placed && attempts < 150) {
        ...
        for (let dy = -minPathDistance; dy <= minPathDistance; dy++) {
          for (let dx = -minPathDistance; dx <= minPathDistance; dx++) {
        ...
        if (!tooCloseToOtherRock) {
          ...
          // Create Rock entity
          const rock = new Rock(x * TILE_SIZE, y * TILE_SIZE, this.grid);
        ...
        while (!placed && attempts < 200) {
          ...
          if (this.grid.isDirt(x, y)) {
            ...
            if (!nearTunnel) {
              ...
              const notOnRock = !this.rocks.some(existingRock => {
                return existingRock.gridX === x && existingRock.gridY === y;
              });
    ...
    for (let i = 0; i < numEnemies; i++) {
      ...
      while (!placed && attempts < 100) {
        ...
        if (!tooClose) {
          ...
          if (horizontal) {
            this.grid.clearHorizontalTunnel(x, Math.min(x + length, GRID_WIDTH - 2), y);
          } else {
            this.grid.clearVerticalTunnel(x, y, Math.min(y + length, GRID_HEIGHT - 2));
          }
  ...
  getEnemySpawnPosition(enemyIndex) {
    ...
    if (enemyIndex < this.enemyTunnels.length) {
      ...
      const offsetX = tunnel.horizontal ? Math.floor(tunnel.length / 2) : 0;
      const offsetY = tunnel.horizontal ? 0 : Math.floor(tunnel.length / 2);
