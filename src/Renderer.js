import { CANVAS_WIDTH, CANVAS_HEIGHT, TILE_SIZE, COLORS, TILE_TYPES, DIRECTIONS } from './utils/constants.js'
...
export class Renderer {
	...
	drawPlayer(player) {
		...
		if (this.spritesLoaded) {
			...
			if (sprite && sprite.complete) {
				...
				this.ctx.drawImage(sprite, -TILE_SIZE / 2, -TILE_SIZE / 2, TILE_SIZE, TILE_SIZE)
	...
	drawEnemy(enemy) {
		...
		if (enemy.type === 'pooka') {
			...
			this.ctx.fillStyle = enemy.isGhosting ? 'rgba(231, 76, 60, 0.5)' : '#e74c3c'
			this.ctx.beginPath()
			this.ctx.arc(centerX, centerY, 6, 0, Math.PI * 2)
			this.ctx.fill()
		} else if (enemy.type === 'fygar') {
			// Green circle for Fygar
			this.ctx.fillStyle = enemy.isGhosting ? 'rgba(46, 204, 113, 0.5)' : '#2ecc71'
			this.ctx.beginPath()
			this.ctx.arc(centerX, centerY, 6, 0, Math.PI * 2)
			this.ctx.fill()
		}
	...
	drawUI(scoreManager) {
		...
		// Score (left)
		this.drawText(`${scoreManager.score}`, padding, padding + fontSize + 4, {
			size: fontSize,
		...
		// Lives (right)
		this.drawText(`${scoreManager.lives}`, CANVAS_WIDTH - padding, padding + fontSize + 4, {
			size: fontSize,
			color: COLORS.TEXT_WHITE,
			align: 'right',
		})
		this.drawText('LIVES', CANVAS_WIDTH - padding, padding + fontSize + 16, {
			size: 6,
			color: COLORS.TEXT_YELLOW,
			align: 'right',
		})
		...
		// Level (center)
		this.drawText(`LVL ${scoreManager.level}`, CANVAS_WIDTH / 2, padding + fontSize + 4, {
			size: fontSize,
			color: COLORS.TEXT_YELLOW,
			align: 'center',
		})
